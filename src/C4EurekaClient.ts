...
export default class C4EurekaClient {
    ...
    init(eureka : EurekaConfig, instanceInfo : InstanceInfo) {
        ...
        this.m_Client   = new EurekaClient.Eureka({
            instance : <EurekaClient.EurekaClient.EurekaInstanceConfig>instanceInfo,
            eureka   : <EurekaClient.EurekaClient.EurekaClientConfig>eureka
        });
    }
    ...
    async start() {
        ...
        await new Promise((resolve, reject) => {
            if (Self.m_Client !== null) {
                (<any>Self.m_Client).start((err : any) => {
                    if (err) {
                        return reject(err);
                    }
                    resolve();
    ...
    async stop() {
        ...
        await new Promise((resolve, reject) => {
            if (Self.m_Client !== null) {
                (<any>Self.m_Client).stop((err : any) => {
                    if (err) {
                        return reject(err);
                    }
                    resolve();
